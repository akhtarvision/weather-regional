...
from climax.loraqkv import _LoRA_qkv
# from climax.loramlp import _LoRA_mlp
...
class ClimaX(nn.Module):
    ...
    def __init__(
        ...
        # self.w_Asfc = []  # These are linear layers
        # self.w_Bsfc = []
        ...
        for t_layer_i, blk in enumerate(self.blocks):
            ...
            # # self.dimlp = blk.mlp.fc1.out_features
            # self.dimlp = blk.mlp.fc1.in_features
            # w_a_linear_fc1 = nn.Linear(self.dimlp, r, bias=False)
            # w_b_linear_fc1 = nn.Linear(r, self.dimlp, bias=False)
            # # w_a_linear_fc2 = nn.Linear(self.dimlp, r, bias=False)
            # # w_b_linear_fc2 = nn.Linear(r, self.dimlp, bias=False)
            # self.w_Asfc.append(w_a_linear_fc1)
            # self.w_Bsfc.append(w_b_linear_fc1)
            # # self.w_Asfc.append(w_a_linear_fc2)
            # # self.w_Bsfc.append(w_b_linear_fc2)

            # torch.nn.init.kaiming_uniform_(w_a_linear_q.weight, a=math.sqrt(5))
            # torch.nn.init.kaiming_uniform_(w_a_linear_v.weight, a=math.sqrt(5))
            # torch.nn.init.zeros_(w_b_linear_q.weight)
            # torch.nn.init.zeros_(w_b_linear_v.weight)
    ...
    def reset_parameters(self) -> None:
        ...
        # for w_A in self.w_Asfc:
        #     nn.init.kaiming_uniform_(w_A.weight, a=math.sqrt(5))
        # for w_B in self.w_Bsfc:
        #     nn.init.zeros_(w_B.weight)
